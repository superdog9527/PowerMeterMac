...
- 0.600–5.000 V 输出设置，1 mV 步进；输出或采集运行时可直接切换电压
...
当 GitHub Release 正式发布时，发布工作流会检出该 Release 的标签，把标签写入应用版本号，重新测试并构建应用，生成 `Power-Meter-macOS-*.dmg` 和对应的 SHA-256 文件，并自动附加到 Release。对于工作流建立前已有的 Release，可手动运行 `Publish macOS DMG` 并填写 `release_tag` 补发附件。当前应用使用临时签名，尚未进行 Apple Developer ID 签名和公证。
...
采集过程中修改电压时，应用会在后台短暂停止硬件采样、排空旧电压数据、保持输出状态并设置新电压，再使用新电压对应的校准参数恢复采样。波形和整轮统计不会重置；CSV 为每个样本记录采样时的实际设定电压和对应功耗。设备数据没有硬件时间戳，因此切换期间的短暂采样空档不会计入按样本序号计算的时间轴。
